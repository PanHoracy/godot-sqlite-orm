...
class ProductReviewORMSelect:
	...
	var _parent_table: ProductReviewORM
	
	
	func _init(parent_table: ProductReviewORM) -> void:
	...
	func group_by(...columns: Array) -> ProductReviewORMSelect:
		super.group_by(columns)
		return self
	
	
	func having(condition: ORMCondition) -> ProductReviewORMSelect:
...
func get_all_columns() -> Array[ORMColumn]:
	return [title, content, product_id, id, ]
